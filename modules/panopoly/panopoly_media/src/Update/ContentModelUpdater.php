...
class ContentModelUpdater {
  ...
  public function cleanup() {
    ...
    // Removes the temporary oembed field from the video type.
    if ($field = FieldConfig::load('media.video.field_media_oembed_video')) {
      $field->delete();
    }
  }
  ...
  public function convertMedia(&$context) {
    ...
    $id = array_shift($context['sandbox']['media_ids']);
    if ($id) {
  ...
  protected function installFields() {
    ...
    // Temporarily add the oembed field to video type. This is needed to perform
    // the media type conversion and ends up being removed at the end of the
    // update.
    if (!$this->configStorage->exists('field.field.media.video.field_media_oembed_video')) {
  ...
  protected function updateFormDisplay($id) {
    ...
    switch ($id) {
      case 'media.panopoly_media_file.default';
        ...
        $this->hideEntityFormPath($content, $hidden);
   ...
   *
   * @param array $content
   *   The content configuration array.
   * @param array $hidden
   *   The hidden configuration array.
   */
  protected function hideEntityFormPath(array &$content, array &$hidden) {
  ...
  protected function convertMediaEntity($id) {
    ...
    switch ($media->bundle()) {
      case 'file':
        $media->set('bundle', 'panopoly_media_file');
        $media->save();
        break;

      case 'image':
        $media->set('bundle', 'panopoly_media_image');
        $media->save();
        break;

      case 'video':
        $value = $media->get('field_media_video_embed_field')->value;
        $media->set('bundle', 'panopoly_media_remote_video');
  ...
  protected function getConfigFileStorage() {
    ...
    if ($this->configFileStorage === NULL) {
      $configPath = drupal_get_path('module', 'panopoly_media') . '/' . InstallStorage::CONFIG_INSTALL_DIRECTORY;
